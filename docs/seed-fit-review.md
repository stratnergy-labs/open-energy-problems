...
| EP-901 Open Optimization Stack for Energy Models | HiGHS, SCIP, CBC, OR-Tools, Pyomo, and JuMP are plausible enabling-infrastructure fits; status should remain `PARTIAL` until energy-specific benchmark coverage is reviewed. |
...
## Energy-Enabling Infrastructure Review Note

Added `EP-901 Open Optimization Stack for Energy Models` to cover non-energy-specific open software that energy modelling depends on. This keeps the register open to projects such as HiGHS, SCIP, CBC, OR-Tools, Pyomo, and JuMP without turning the register into a general software directory.

Fit rule:

- Include non-energy projects only when the problem card explains the energy-specific context.
- Do not treat open solver availability as proof that a given energy workload is reproducible.
- Keep status at `PARTIAL` until representative energy benchmark cases show which workloads are practical on open stacks.
- Record solver, modelling layer, version, licence, input data, and model-instance caveats separately.
